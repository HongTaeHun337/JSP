	...
	private static void m2() {
		...
		try {
			...
			if(!conn.isClosed()) {
				...
				String sql1 = "insert into tblAddress(seq, name, age, gender, tel, address, regdate)values (seqAddress.nextval, '강아지', 20, 'm', '010-1234-5678', '서울시 강남구 대치동', default)";
				
				//쿼리 실행
				//- 반환값O: ResultSet executeQuery()
				//- 반환값X: void executeUpdate()
				
				int result = stat.executeUpdate(sql1); //Ctrl + Enter 역할
...
String sql2 = "insert into tblAddress(seq, name, age, gender, tel, address, regdate)values (seqAddress.nextval, '고양이', 20, 'f', '010-1234-5678', '서울시 강남구 대치동', default)";
				
				//쿼리 실행
				//- 반환값O: ResultSet executeQuery()
				//- 반환값X: void executeUpdate()
				
				result = stat.executeUpdate(sql2); //Ctrl + Enter 역할
